...
export async function onRequestGet({ request, env }) {
  ...
  const user = await env.DB.prepare(
    'SELECT id, COALESCE(custom_display_name, display_name) as display_name, avatar_url, favorite_pokemon_id, name_color, name_effect FROM users WHERE id = ?'
  ...
  return json({
    loggedIn: true,
    user: { id: user.id, name: user.display_name, avatar: resolveAvatarUrl(user), nameColor: user.name_color || null, nameEffect: user.name_effect || null }
  });
}
